...
class ErrorHandler implements Exception {
  ...
  /// 🔥 Inilah method baru yang kamu panggil di Service
  static Failure map(dynamic error) {
    return _mapError(error);
  }
}

/// Fungsi internal untuk mengembalikan Failure
Failure _mapError(dynamic error) {
  if (error is DioException) {
    return _handleErrorDio(error, ['message'], false);
  } else if (error is FormatException) {
    return DataSource.PARSING_ERROR.getFailure();
  } else if (error is TypeError) {
    return DataSource.WRONG_DATA_TYPE.getFailure();
